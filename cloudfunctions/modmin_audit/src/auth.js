...
function getJwtSecret() {
  ...
  if (!secret || secret === 'modmin_dev_secret_change_in_production') {
    throw new Error('[modmin_audit] MODMIN_JWT_SECRET 环境变量未配置或仍为开发占位值，拒绝启动')
  }

  if (secret.length < 32) {
    throw new Error('[modmin_audit] MODMIN_JWT_SECRET 长度不足 32 字符，安全性不达标')
  }
...
function createAuthHelpers({ jwtSecret, fail }) {
  ...
  function requireSuperAdmin(event) {
    ...
    if (payload.roleCode !== 'role_super_admin') {
      return fail(event, 40301, '仅超级管理员可查看操作日志')
    }
